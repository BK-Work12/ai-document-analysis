<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-gray-900 leading-tight">Edit Document Type</h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">Update Document Type</h3>
                    <p class="text-sm text-gray-600 mt-1">Make changes to the document type configuration</p>
                </div>

                <!-- Form Content -->
                <div class="p-6">
                    <form action="{{ route('admin.document-types.update', $documentType->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" id="doc_type" name="doc_type" value="{{ old('doc_type', $documentType->doc_type) }}" 
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" 
                                ...
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 transition-all resize-none">{{ old('description', $documentType->description) }}</textarea>
                            @error('description')
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div class="flex items-start gap-3 p-4 border border-gray-200 rounded-lg hover:bg-blue-50 transition-colors">
                                <input type="checkbox" id="required" name="required" value="1" {{ old('required', $documentType->required) ? 'checked' : '' }}
                            ...
                            <div class="flex items-start gap-3 p-4 border border-gray-200 rounded-lg hover:bg-green-50 transition-colors">
                                <input type="checkbox" id="active" name="active" value="1" {{ old('active', $documentType->active) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $documentType->sort_order) }}"
