<x-app-layout>
    <x-slot name="header">Client Chats - {{ $user->name }}</x-slot>
        ...
        <div class="w-64 bg-white border-r border-gray-200 overflow-y-auto">
            <div class="p-4 space-y-2">
                @forelse($documents as $doc)
                    <a href="{{ route('admin.chats.show', [$user->id, $doc->id]) }}"
                ...
                @empty
                    <p class="text-center text-gray-500 py-8">No documents submitted.</p>
                @endforelse
            </div>
        </div>
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="text-right">
                            <span class="px-4 py-2 rounded-full {{ 
                                ...
                                ($selectedDocument->status === 'needs_correction' ? 'bg-red-100 text-red-800' :
                                ($selectedDocument->status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                                'bg-gray-100 text-gray-800'))
                            }} font-semibold text-sm">
                                {{ ucfirst(str_replace('_', ' ', $selectedDocument->status)) }}
                            </span>
